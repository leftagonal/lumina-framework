...
namespace lumina::renderer {
    ...
    namespace accessors {
        struct PresenterAccessor {
            [[nodiscard]] static GLFWwindow*& window(Presenter& presenter, const WindowHandle& handle) {
                return presenter.windows_[handle.id()].resource;
            }

            [[nodiscard]] static VkSurfaceKHR& surface(Presenter& presenter, const WindowHandle& handle) {
                return presenter.surfaces_[handle.id()];
            }

            [[nodiscard]] static VkSwapchainKHR& swapchain(Presenter& presenter, const WindowHandle& handle) {
                return presenter.swapchains_[handle.id()];
            }

            [[nodiscard]] static const GLFWwindow* const& window(const Presenter& presenter, const WindowHandle& handle) {
                return presenter.windows_[handle.id()].resource;
            }

            [[nodiscard]] static const VkSurfaceKHR& surface(const Presenter& presenter, const WindowHandle& handle) {
                return presenter.surfaces_[handle.id()];
            }

            [[nodiscard]] static const VkSwapchainKHR& swapchain(const Presenter& presenter, const WindowHandle& handle) {
                return presenter.swapchains_[handle.id()];
            }
        };
    }
